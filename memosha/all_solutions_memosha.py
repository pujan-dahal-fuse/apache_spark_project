...
spark = SparkSession.builder.appName('weather_history')    .config('spark.driver.extraClassPath', '/usr/lib/jvm/java-11-openjdk-amd64/lib/postgresql-42.5.0.jar')    .getOrCreate()

historical_weather_df = spark.read.option("header",True)      .csv("../cleaned_data/cleaned_weather_data.csv")

historical_weather_df.show()

historical_weather_df.printSchema()

country_continent_df = spark.read.option("header",True)      .csv("../cleaned_data/cleaned_country_continent.csv")
...
stddev_min_temp_may_df = min_temp_may_df                            .groupBy('country')                            .agg(fun.stddev('min_temp').alias('stddev'))                            .orderBy('country')                            .select('country',fun.round('stddev', 1).alias('stddev_min_temp_may'))
...
ranked_df = historical_weather_df                .withColumn('dense_rn', fun.dense_rank().over(window_spec))                .filter('dense_rn <= 5')                .filter('country == "Belgium"')
...
avg_temp_df = historical_weather_df                .filter(fun.year('date') == 2021)                .groupBy('country')                .agg(fun.mean('tavg').alias('avg_temp'))                .orderBy('country')
...
diff_global_avg_temp_df = avg_temp_df                            .withColumn('year', fun.lit(2021))                            .withColumn('avg_temp',fun.round('avg_temp', 1))                            .withColumn('global_avg_temp', fun.lit(global_avg_temp))                            .withColumn('difference', fun.round(col('avg_temp') - col('global_avg_temp'), 1))
...
canada_december_2020_df = historical_weather_df                        .filter(fun.year('date') == 2020)                        .filter(fun.month('date') == 12)                        .filter('country == "Canada"')                        .withColumn('rolling_avg', fun.mean('tavg').over(window_spec))                        .select('date', 'country', 'tavg', fun.round('rolling_avg', 1).alias('rolling_avg'))
...
hottest_coldest_belgium_df = historical_weather_df        .filter('country == "Belgium"')        .filter(fun.year('date') == 2019)        .withColumn('min_rank', fun.dense_rank().over(window_spec1))        .withColumn('max_rank', fun.dense_rank().over(window_spec2))        .withColumn('is_min_or_max', fun.when(col('min_rank') == 1, fun.lit('min')).when(col('max_rank') == 1, fun.lit('max')))        .filter((col('min_rank') == 1) | (col('max_rank') == 1))        .select(fun.to_date('date').alias('date'), 'country', 'tmin', 'tmax', 'is_min_or_max') 
...
canada_2021_windiest_df=historical_weather_df        .filter('country == "Canada"')        .filter(fun.year('date') == 2021)        .withColumn('max_rank', fun.dense_rank().over(window_spec))        .select(fun.to_date('date').alias('date'), 'country', 'wspd')
...
highest_temp_continent_df = joined_df                            .withColumn('dense_rnk', fun.dense_rank().over(window_spec))                            .filter('dense_rnk == 1')                            .select('continent', 'date', historical_weather_df['country'], col('tmax').alias('max_temp'))
